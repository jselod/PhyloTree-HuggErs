...
#CPK: not need to set wd bc your data are in the project dir. [-1]. Changing:
...
#CPK, we really need a boxplot, bc we asked for residuals plotted against this discrete factor (i.e, ecomorph)[-1]. Something like:

mod.tibble %>%
  ggplot(aes(Ecomorph,PH.res)) + geom_boxplot() 

mod.tibble %>%
  ggplot(aes(Ecomorph,PD.res)) + geom_boxplot() 
...
#CPK: Nice work, esp with the residuals and boxplot for the last few prompts.
